...
#region Install Electron & nodejs
...
if((test-path $rec_txt)){
    #check npm install and install npm/electron
...
#endregion Install Electron & nodejs       
...
###Code for ping and copy all machine################
<#
...
#    
#}catch{
#    Write-Output "Electron Open..."
#}


####ping and copy for one machine
 ...
 if($mnend){
    ...
    if($getcsv.MachineName -eq $mnend){
     #if($mname -in  $passlist2){
      $newcsv="\\192.168.60.16\srvprj\Inventec\Dell\Matagorda\07.Tool\_AutoTool_Monitor\logs\$($mnend)\logs_timemap.csv"
      ...
      $checklogs=((import-csv $newcsv).TC).count
       if($checklogs -gt 0){
                ...
                $Last_TC = $lastlogs."TC"
                $Last_Step = $lastlogs."Step_No"
                $Last_Program = $lastlogs."program"
                $Last_RunTime = $lastwt
            }
            ...
            if($checklogsold -gt 0){
                ...
                #$getcsv.Last_RunTime=($getcsvsold|?{$_.MachineName -eq $mnend})."Last_RunTime"


                $Last_TC = ($getcsvsold|?{$_.MachineName -eq $mnend})."Last_TC"
                $Last_Step = ($getcsvsold|?{$_.MachineName -eq $mnend})."Last_Step"
                $Last_Program = ($getcsvsold|?{$_.MachineName -eq $mnend})."Last_Program"
                $Last_RunTime = ($getcsvsold|?{$_.MachineName -eq $mnend})."Last_RunTime"
    ...
    foreach($D in $Data)
    {
        ...
        <#
            ...
            foreach($col in $columnnms){
                ...
                #if($D.IPv4IP -eq $ipend -and $col -eq "log_folder"){
                if($col -eq "log_folder"){
                    
                    $insertdata+= "<td> <a id='explorer$id' href='$($D.$col)' style='color:gray; text-decoration: none; cursor: not-allowed; pointer-events: none;'>$($D.$col)</a><p id='load$id' style='display:none;'>Loading...</p></td>"
                    <#
                    if($checkconnect.status -match "success"){
                    #if($faillist -eq $D.MachineName){
        ...
        #foreach($D in  $Data){
        #    if($D.MachineName -eq $mnend){
        #        $Last_TC = $D.Last_TC
        #        $Last_Step = $D.Last_Step
        #        $Last_Program = $D.Last_Program
        #        $Last_RunTime = $D.Last_RunTime
        #    }
        #}

        #--------RemoteDetect--------------------------------------------------------------
        $html = Get-Content -Path "$scriptRoot\interface.html"
        
        # 使用正则表达式提取所有<td>标签
        $trTags = [regex]::Matches($html, '<tr>.*?</tr>') | ForEach-Object { $_.Value }

        # 打印提取的<td>标签内容
        foreach ($trTag in $trTags) {
       
            if($trTag -match $mnend){
                $tdTags = [regex]::Matches($trTag, '(?<=<td>)(.*?)(?=<\/td>)')

                $tagposition = $tdTags[0].Index + $tdTags[0].Length
                $tagpositionLogFolder = $tdTags[5].Index + $tdTags[5].Length
                ...
                if($tdTags[10].Value -notmatch $Last_RunTime){
                    $originString = $originString.Insert( $tdTags[10].Index,$Last_RunTime)
                }
                if($tdTags[9].Value -notmatch $Last_Program){
                    $originString = $originString.Insert( $tdTags[9].Index,$Last_Program)
                }
                if($tdTags[8].Value -notmatch  $Last_Step ){
                    $originString = $originString.Insert( $tdTags[8].Index,$Last_Step)
                }
                if($tdTags[7].Value -notmatch $Last_TC){
                    $originString = $originString.Insert( $tdTags[7].Index,$Last_TC)
                }    
                

                if($originString -match "Failed"){
                    $flag = 0 
                    $replaceString = $originString.Remove($tagposition-6,6)
                    $tagpositionLogFolder -= 6
                }else{
                    $flag = 1
                    $replaceString = $originString.Insert( $tagposition,"Failed")
                    $tagpositionLogFolder += 6
                }

                if($originString -match "style"){
      
                    if($flag2){
                        #$replaceString2 = $replaceString.Remove($tagpositionLogFolder+37+$mnend.Length+20,86)
                        $replaceString2 = $replaceString.Remove($replaceString.IndexOf("\\$mnend")+$mnend.Length+20,86)
                        
                    }
                }else{
      
                    if(!$flag2){
                        $replaceString2 = $replaceString.Insert($replaceString.IndexOf("\\$mnend")+$mnend.Length+20,"style='color:gray; text-decoration: none; cursor: not-allowed; pointer-events: none;'")
                    }
                }


                $originString = $trTag
                #$trTag = $trTag[0..$tagposition] + $trTag[($tagposition+1)..($trTag.Length-1)]
                #$trTag.Replace("<td><input id='Remote5' type='button' value='Connect' /></td>","<td><input id='Remote5' type='button' value='Connect' />Failed</td>")
            }
        }
            ...
            if($line -match $mpend){
                if($checkconnect.status -match "success"){
                    if(!$flag){
                        ...
                        if($replaceString2){
                            $html[$i] = $html[$i].Replace($originString,$replaceString2)
                        }else{
                            $html[$i] = $html[$i].Replace($originString,$replaceString)
                        }
                        ...
                        #$originString2 = $html[$i]

                        #if(!$flag2){
                        #    $html[$i] = $html[$i].Replace($originString2,$replaceString2)
                        #}else{
                        #    $html[$i] = $html[$i].Replace($originString2,$replaceString2)
                        #}
                    }
                ...
                #$line.Replace($originString,$replaceString)
            }
            $i += 1
        }
        #$html = $html[0..$html.IndexOf("<tbody>")] + $insertdata + $html[($html.IndexOf("</tbody>")-1)..$html.Length]
        ...
        # $ipend = "192.168.2.141"
        # $mnend = "Win10_NV_T1000"

        # $ipend = "192.168.2.64"
        # $mnend = "SIT4"

        # $ipend = "192.168.2.133"
        # $mnend = "Win10_SSD_AMD_6300"

        # $ipend = "192.168.2.83"
        # $mnend = "M22"

        Write-Output $checkconnect.Status
    }
}
else{
